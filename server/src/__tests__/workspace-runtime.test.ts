...
// but the workspace runtime under test realpaths the roots it hands back. Comparing
// a raw `mkdtemp` path against a realpath'd runtime path can therefore never pass on
// a Mac. Normalize at creation so every path derived from a temp root is already in
// its realpath form and assertions compare like with like on every platform.
